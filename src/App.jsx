...
export default function App() {
  ...
  useEffect(() => {
    try {
      const e = lsGet('fx:entries');
      const ta = lsGet('fx:tags');
      const s = lsGet('fx:cfg');
      if (e) setEntries(JSON.parse(e));
      if (ta) setTags(JSON.parse(ta));
      if (s) setCfg(JSON.parse(s));
    } catch {}
    setLoading(false);
  }, []);

  const saveE = (d) => lsSet('fx:entries', JSON.stringify(d));
  const saveT = (d) => lsSet('fx:tags', JSON.stringify(d));
  const saveC = (d) => lsSet('fx:cfg', JSON.stringify(d));
  ...
  const importTxt = (txt) => {
    const chunks = txt.split(/(?=^\$)/m).map(s => s.trim()).filter(Boolean);
    if (!chunks.length) { notify('未识别到内容，请确认格式'); return; }
    const news = chunks.map(chunk => {
      let title = '';
      const m = chunk.slice(0, 300).match(/[""「『]([^""」』]{2,20})[""」』]/);
      if (m) title = m[1];
      if (!title) {
        const m2 = chunk.match(/"([^"]{2,20})"/);
        if (m2) title = m2[1];
      }
      if (!title) title = '未命名番外';
      return { id: uid(), title, status: 'unwritten', tags: [], prompt: chunk, content: '', note: '', favorite: false, createdAt: Date.now() };
    });
    const next = [...news, ...entries]; setEntries(next); saveE(next);
      ...
      <div style={{ ...appBg, paddingBottom: 32 }}>
        <StickyHeader>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {iconBtn(() => setView('list'), ICONS.back, th.textM, 22)}
            ...
            {iconBtn(() => toggleFav(e.id), ICONS.fav, e.favorite ? th.heart : th.borderL, 20, e.favorite)}
            {iconBtn(() => setView('edit'), ICONS.edit, th.textM, 20)}
          </div>
        </StickyHeader>
        <div style={{ padding: '20px 16px' }}>
          ...
          <button onClick={() => { if (window.confirm('确定删除这篇番外？')) deleteE(e.id); }}
